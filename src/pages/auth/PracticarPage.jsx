...
export default function PracticarPage() {
  ...
  async function saveSession() {
    try {
      await supabase.from('intentos').insert(
        exam.intentos.map(i => ({ ...i, usuario_id: usuario.id }))
      );
  ...
  async function handleStart() {
    ...
    if (!data?.length) { setSetupError('No hay preguntas disponibles con estos filtros. Prueba con otras opciones.'); return; }
    const shuffled = data.sort(() => Math.random() - .5).slice(0, config.numPreguntas);
