...
export default function ExamenPage() {
  ...
  // BUG 1 FIX: guard para que persistSession solo se ejecute UNA vez por sesión.
  // Sin esto, si el flujo pasa por 'review' antes de 'result', se llamaba dos veces:
  // una al llegar a 'review' y otra al llegar a 'result' — duplicando exam_responses
  // y doblando times_wrong / times_correct en user_question_state.
  const persistedRef = useRef(false);

  useEffect(() => {
    if (exam.phase === 'exam') {
      timerRef.current = setInterval(() => exam.tick(), 1000);
      persistedRef.current = false; // reset al iniciar un nuevo examen
    } else {
  ...
  // Guardar al finalizar — solo en 'result', nunca en 'review'
  useEffect(() => {
    if (exam.phase === 'result' && exam.sessionId && !persistedRef.current) {
  ...
  async function handleStart() {
    ...
    // BUG 4 FIX: mode era siempre 'study' independientemente del modo real.
    // Las sesiones de repaso y errores se registraban incorrectamente en BD.
    const session = await createSession({
      userId: profile.id, mode: cfg.mode || 'study',
