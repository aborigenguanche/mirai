...
export default function PlanDiaPage() {
  ...
  // BUG 5 FIX: deps vacío [] significa que load() solo corre al montar.
  ...
  async function load() {
    ...
    const allSpecs   = await fetchSpecialties();
    const specDetails = allSpecs
      .filter(s => specs.includes(s.id))
  ...
  if (diasAlMir !== null && diasAlMir < 30) {
    ...
    coachMsg  = 'Empieza tu primera sesión para ver tus estadísticas personalizadas.';
    coachType = 'neutro';
  } else {
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-5 mb-5">
        <div className="lg:col-span-2 flex flex-col gap-4">
          {[
            ...
            {
              icon:'✨', title:'Preguntas nuevas',
              desc: primeraSpec ? `Prioridad: ${primeraSpec.name}` : 'Todas las especialidades',
        ...
        <div className="flex flex-col gap-5">
          ...
          <div className={`rounded-xl p-5 border ${us.bg} ${us.border}`}>
            ...
            {diasAlMir !== null ? (
              ...
              <div>
                ...
                <Link to="/app/perfil" className={`text-xs font-semibold ${us.text} hover:underline`}>
                  Configurar fecha →
                </Link>
              </div>
            )}
          ...
          <Card>
            ...
            <div className="flex flex-col gap-3">
              {[
                { label:'Tasa de acierto', val: tasa > 0 ? `${tasa}%` : '—',   color: tasa>=65?'text-pulse-dim':tasa>=50?'text-amber-500':tasa>0?'text-red-400':'text-slate-300' },
                { label:'Racha actual',    val: racha > 0 ? `${racha} días` : '—', color: racha>=3?'text-amber-500':'text-slate-400' },
              ...
              <div className="pt-2 border-t border-border">
                ...
                <div className="h-2 bg-sky-100 rounded-full overflow-hidden relative">
                  <div className="h-full bg-gradient-to-r from-sky-500 to-pulse rounded-full transition-all duration-700"
                ...
                <div className="flex justify-between text-[0.6rem] font-mono mt-1">
                  <span className={tasa>=65?'text-pulse-dim font-bold':'text-slate-400'}>
                    {tasa > 0 ? `${tasa}%` : 'Sin datos'}
                  </span>
